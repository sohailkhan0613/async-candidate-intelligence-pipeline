...
export async function markSuccess(): Promise<void> {
  await redis.multi().set(STATE_KEY, "CLOSED").del(FAILURES_KEY).del(OPENED_AT_KEY).exec();
}

export async function markFailure(): Promise<void> {
  const now = Date.now();
  await redis.rpush(FAILURES_KEY, now);
  const all = (await redis.lrange(FAILURES_KEY, 0, -1)).map(Number);
  const recent = all.filter((t) => now - t <= FAILURE_WINDOW_MS);
...
export async function getCircuitState(): Promise<{ state: string; openedAt: string | null }> {
  const [state, openedAt] = await Promise.all([redis.get(STATE_KEY), redis.get(OPENED_AT_KEY)]);
  return { state: state ?? "CLOSED", openedAt };
}
